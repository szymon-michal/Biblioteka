...
export function MembersPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const guess = guessResourcePaths(doc).members
      const effective = guess || '/members'
      setPath(effective)
      try {
  ...
  const cols = useMemo(() => {
    ...
    const preferred = ['id', 'email', 'username', 'firstName', 'lastName', 'name', 'phone', 'status', 'createdAt']
    keys.sort((a, b) => {
        ...
        {loading ? (
          ...
          <div>
            ...
            <div className={styles.muted} style={{ marginTop: 8 }}>
              Use API Explorer to locate the correct members endpoint.
            </div>
          </div>
        ) : rows.length === 0 ? (
          <div className={styles.text}>No members.</div>
        ) : (
