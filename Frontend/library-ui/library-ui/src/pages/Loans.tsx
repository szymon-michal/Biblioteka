...
export function LoansPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const guess = guessResourcePaths(doc).loans
      const effective = guess || '/loans'
      setPath(effective)
      try {
  ...
  const cols = useMemo(() => {
    ...
    const preferred = ['id', 'bookId', 'memberId', 'readerId', 'dueDate', 'returned', 'status', 'createdAt']
    keys.sort((a, b) => {
  ...
  return (
    <div className={styles.page}>
      <div className={styles.sectionHeader}>
        <div>
          <h2 className={styles.h2}>Loans</h2>
          <div className={styles.muted}>
            Auto-detected endpoint: <code>{path}</code>
          </div>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div>
            ...
            <div className={styles.muted} style={{ marginTop: 8 }}>
              Use API Explorer to find the correct endpoint if needed.
            </div>
          </div>
        ) : rows.length === 0 ? (
          <div className={styles.text}>No loans.</div>
        ) : (
